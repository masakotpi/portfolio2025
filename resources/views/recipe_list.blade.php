    ...
    <div class="card-body">
      ...
      <div class="input-group flex-nowrap my-4">
        ...
        <select name="type" class="form-control">
          <option value="" disabled {{ $request->type == null ? 'selected' : '' }}>選択してください</option>
      ...
      <div class="input-group flex-nowrap">
        ...
        <select name="name" class="form-control">
          <option value="" disabled {{ $request->name === null ? 'selected' : '' }}>選択してください</option>
          @foreach($recipes['names'] as $key => $value)
              <option value="{{ $key }}" {{ $request->name == $key ? 'selected' : '' }}>{{ $value }}</option>
          @endforeach
      </select>
      </div>
      </form>
